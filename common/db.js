...
const getCurBoard = async (walletAddress) => {
...
const stopGame = async (data) => {
  const depositResult = await checkDeposit(data);
  console.log("depositResult", depositResult);
  if (depositResult.walletAddress !== data.walletAddress) return;
  const collection = db.collection("boards");
  ...
  const tempWalletAddress = curBoard.walletAddress;
  const tempMulti = multi * curDeposit.bettingAmount;

  // save history
  let userData = null;
  userData = await getUserData({ walletAddress: data.walletAddress });
  const hisData = {
    walletAddress: data.walletAddress,
    game: "MinesRush",
    player: userData == null ? data.walletAddress : userData.userName,
    wager: curDeposit.bettingAmount,
    payout: multi * curDeposit.bettingAmount,
    date: getDate(),
  };
  ...
  claimReward(tempWalletAddress, tempMulti);
};
...
const claimReward = async (walletAddress, payout) => {
  const collection = db.collection("withdraw");
  ...
  await collection.insertOne(withdrawData);
...
const userFailResetAll = (data) => {
  ...
  boardcollection.deleteOne({ walletAddress: data.walletAddress });
...
const userClickedCoin = async (data) => {
  ...
  if (result) {
    ...
    const hisData = {
      walletAddress: data.walletAddress,
      game: "MinesRush",
      player: userData == null ? walletAddress : userData.userName,
      wager: curDeposit.bettingAmount,
      payout: multi * curDeposit.bettingAmount,
      date: getDate(),
    };
    console.log("hisData win", hisData);
    saveHistory(hisData);
